...
const SideNav = ({children}) => {
  ...
  return (
    <div className='w-full flex justify-between'>
        ...
        <div className={`${openNav? 'w-[20%]' : 'w-[5%]'} gap-4 h-[100vh] fixed flex-col bg-[#2F8D47] px-2 py-4 ${ openNav? 'max-md:w-[40%]' : 'max-md:w-[20%]'}`}>
        ...
        <main className={`text-black  h-scren flex-1 ${openNav? 'ml-[20%]' : 'ml-[5%]'} py-4`}>{children}</main>
    </div>
  )
}
